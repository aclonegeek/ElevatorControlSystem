...
public class BestElevatorTest extends TestCase {
    public void testBestElevator() {
        ...
        //assert we find an elevator going the same direction with the least stops before
       target = new BestElevator(3,5);
       predicted = scheduler.getBestElevator(6, ElevatorState.MOVING_UP);
       assertEquals(target.elevatorId, predicted.elevatorId);
       assertEquals(target.numFloors, predicted.numFloors);
       
       //assert we find the elevator with least total stops if there is no elevator going same direction
